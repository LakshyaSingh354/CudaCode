...
__global__ void qgemm_kernel_tiled(const int8_t* __restrict__ Wq, const float* __restrict__ Sw,
    ...
    for (int t = 0; t < (K + TILE_SIZE - 1) / TILE_SIZE; t++) {
        // Load Wq tile (M x K)
        int Wq_row = row;
        int Wq_col = t * TILE_SIZE + tx;
        if (Wq_row < M && Wq_col < K) {
            Wq_tile[ty][tx] = Wq[Wq_row * K + Wq_col];
        } else {
            Wq_tile[ty][tx] = 0;
        }
        
        // Load Xq tile (K x N) and subtract zero points
        int Xq_row = t * TILE_SIZE + ty;
        int Xq_col = col;
        if (Xq_row < K && Xq_col < N) {
            int32_t x_val = static_cast<int32_t>(Xq[Xq_row * N + Xq_col]);
            Xq_tile[ty][tx] = x_val - Zx[Xq_col];
        } else {
            Xq_tile[ty][tx] = 0;
        }
        ...
        for (int k = 0; k < TILE_SIZE; k++) {
            acc += static_cast<int32_t>(Wq_tile[ty][k]) * Xq_tile[k][tx];
        }
...
int main() {
    ...
    for (int i = 0; i < warmup_iters; ++i) {
        ...
        qgemm_kernel_tiled<<<grid, block>>>(dWq, dW_scales, dXq, dX_scales, dX_zps, dY, M, K, N);
    }
